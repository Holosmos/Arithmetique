...
\item $\forall x \in G, \exists ! y \in G, x \cdot y = y \cdot x = e$ ;
\item $\forall x,y,z \in G, x \cdot (y\cdot z ) = (x\cdot y) \cdot z$.
\end{enumerate}
}{}

\definition{ 
Si : \[ \forall x,y \in G, x \cdot y = y \cdot x\] alors on dit que le groupe $(G,\cdot)$ est abélien (ou commutatif).
}{}
...
\demonstration{ 
En effet, soient $y,z \in G$ tels que pour $x\in G$ : $x \cdot y = z \cdot x = e$. Alors : \[ z \cdot x \cdot y = z = y.\]
}{}
